...
Before summarizing, call `plaid_connection_status` to check that every bank/card
connection is healthy. If any connection reports `needs_relink`, the numbers below
may be incomplete — that account stopped syncing. Surface this **at the very top of
the email**, above the header, as a short attention banner naming each affected
institution and telling Jenny she needs to reconnect it (e.g. "⚠️ Chase needs to be
reconnected — recent transactions from it may be missing. Reply or open Penny to
relink."). This is an email, so keep it to plain text — no interactive widget. If all
connections are healthy, omit the banner entirely.
...
- Keep the layout simple: the connection banner (only if a connection needs relinking), the header summary, then the three sections in order.
...
2. Write the HTML file to `reports/report-weekly-jenny-latest.html`.
